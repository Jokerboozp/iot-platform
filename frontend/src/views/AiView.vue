...
<template><el-card shadow="never" class="surface-card chat-card"><div ref="log" class="chat-log"><div v-for="(msg,i) in messages" :key="i" class="chat-message" :class="msg.role">{{msg.text}}</div></div><div class="chat-compose"><el-input v-model="question" placeholder="输入运维问题…" @keyup.enter="send" /><el-button type="primary" :loading="sending" @click="send">发送</el-button></div></el-card></template>
