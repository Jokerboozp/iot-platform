...
<template>
  ...
  <el-card shadow="never" class="surface-card table-card">
    <el-table v-loading="loading" :data="registry" stripe>
      ...
      <el-table-column label="操作" width="220" fixed="right" align="center"><template #default="{ row }"><div class="table-actions"><el-button link type="primary" @click="openGuide(row.device.id)">连接指南</el-button><el-button link type="primary" @click="openDebug(row.device.id)">数据联调</el-button></div></template></el-table-column>
