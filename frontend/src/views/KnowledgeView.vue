...
<template>
  <section class="knowledge-hero">
    ...
    <div class="hero-actions">
      <el-tag :type="runtime.persistentIndex ? 'success' : 'warning'" effect="dark">
        {{ runtime.persistentIndex ? 'WEAVIATE 持久索引' : '本地内存索引' }}
      </el-tag>
  ...
  <el-alert
    v-if="!runtime.persistentIndex"
    title="当前使用本地内存索引：文档原文和记录会保存，但 API 重启后检索索引需要重新建立。生产环境请启用 Weaviate。"
    type="warning"
    :closable="false"
    show-icon
  />
  ...
  <div class="knowledge-grid">
    <el-card shadow="never" class="surface-card upload-card">
      <template #header><div class="card-header"><div><strong>上传知识文档</strong><small>单文件最大 32 MiB</small></div><el-tag effect="plain">INDEX</el-tag></div></template>
      ...
      <el-form label-position="top">
        <el-form-item label="关联产品（可选）">
          <el-select v-model="productId" filterable clearable :disabled="!canUpload || uploading" placeholder="不选择表示通用知识">
            <el-option v-for="product in products" :key="product.id" :label="`${product.name || product.id} · ${product.id}`" :value="product.id" />
          </el-select>
          <small class="field-tip">产品列表来自当前租户；不选择时，文档可作为通用知识被工作流检索。</small>
        </el-form-item>
        <div class="metadata-grid">
          <el-form-item label="知识分类">
            <el-select v-model="category" :disabled="!canUpload || uploading">
              <el-option label="设备手册" value="manual" /><el-option label="告警处置 SOP" value="alarm-sop" /><el-option label="运维维修" value="maintenance" /><el-option label="消防规范" value="regulation" /><el-option label="常见问题" value="faq" />
            </el-select>
          </el-form-item>
          <el-form-item label="知识标签">
            <el-select v-model="tags" multiple filterable allow-create default-first-option :disabled="!canUpload || uploading" placeholder="输入标签后回车" />
          </el-form-item>
        </div>
        <el-button type="primary" :loading="uploading" :disabled="!canUpload || !selectedFile" @click="upload">上传并建立索引</el-button>
      </el-form>
    </el-card>

    <el-card shadow="never" class="surface-card table-card documents-card">
      <template #header><div class="card-header"><div><strong>已上传文档</strong><small>仅显示当前租户的知识库记录</small></div><el-button :loading="loading" @click="load">刷新</el-button></div></template>
...
.knowledge-grid { display:grid; grid-template-columns:minmax(300px,.72fr) minmax(560px,1.28fr); gap:14px; align-items:start; }.upload-card :deep(.el-upload) { width:100%; }.upload-card :deep(.el-upload-dragger) { width:100%; padding:28px 12px; }.upload-icon { color:#1677ff; font-size:38px; }.upload-card .el-form { margin-top:18px; }.upload-card .el-form .el-button { width:100%; }.upload-card>.el-alert { margin-bottom:14px; }
.upload-card :deep(.el-select) { width:100%; }.field-tip { display:block; margin-top:5px; color:var(--muted); font-size:10px; line-height:1.5; }.metadata-grid { display:grid; grid-template-columns:.8fr 1.2fr; gap:10px; }.metadata-grid .el-select { width:100%; }.tag-line { display:flex; flex-wrap:wrap; gap:3px; margin-top:4px; }
.document-name { display:flex; align-items:center; gap:9px; }.document-name>.el-icon { width:30px; height:30px; flex:none; color:#1677ff; background:#eaf3ff; border-radius:4px; }.document-name span { min-width:0; }.document-name b { display:block; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }.document-name small { display:block; margin-top:3px; color:var(--muted); font-size:9px; word-break:break-all; }
@media (max-width:1120px) { .knowledge-grid { grid-template-columns:1fr; }.documents-card { order:-1; } }
@media (max-width:640px) { .knowledge-hero { align-items:flex-start; flex-direction:column; }.knowledge-hero p { line-height:1.6; }.hero-actions { width:100%; justify-content:space-between; }.knowledge-stats { grid-template-columns:1fr; }.documents-card { overflow:hidden; } }
</style>
