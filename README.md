...
这是一个模块化 Go 平台，负责产品、协议包、设备注册与凭证、HTTP/MQTT 数据接入、原始报文证据链、独立解析、分层存储、在线状态、消防告警、视频接入、回放、AI/RAG、MCP 工具和统一 API。本项目可以独立完成设备管理与数据接入，不依赖 JetLinks，也不包含 JetLinks 协议包代码。
